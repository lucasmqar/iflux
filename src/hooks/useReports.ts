...
export const useMarkCompanyAsPaid = () => {
  ...
  return useMutation({
    mutationFn: async ({ companyUserId, orders }: { companyUserId: string; orders: any[] }) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select(`
          *,
          company_profiles:company_user_id(company_name),
          driver_profiles:driver_user_id(user_id)
        `)
      ...
      // Get driver names for company view
      if (!isDriver && data) {
        ...
        return data.map(item => ({
          ...item,
          driver_name: profileMap.get(item.driver_user_id) || 'Desconhecido',
          company_name: (item as any).company_profiles?.company_name || 'Desconhecida',
        }));
      }
